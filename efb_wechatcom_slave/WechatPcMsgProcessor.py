...
from .MsgDecorator import efb_text_simple_wrapper, efb_image_wrapper, efb_msgType49_xml_wrapper, efb_location_wrapper
...
class MsgProcessor:
    ...
    def text_msg(msg: dict):
        ...
        return efb_text_simple_wrapper(html.escape(msg['message']))

    # 自定义动画表情
    @staticmethod
    def emojipic_msg(msg: dict):
        m = re.search(r'cdnurl\s?=\s?"(.*?)"', msg['message'])
        if m == None:
            m = re.search(r'thumburl\s?=\s?"(.*?)"', msg['message'])
        if m :
            try:
    ...
    def image_msg(msg: dict):
        if 'base64Content' in msg:
            try:
                file = tempfile.NamedTemporaryFile()
                with urlopen(msg['base64Content']) as response:
                    data = response.read()
                    file.write(data)
                return efb_image_wrapper(file)
            except Exception as e:
    ...
    def msgType49_xml_msg(msg: dict):
        return efb_msgType49_xml_wrapper(msg['message'])
    ...
    def trans_emoji(msg: dict):
        ...
        for emoji_key in emojis:
            msg['message'] = msg['message'].replace(emoji_key, emojis[emoji_key])
        return msg
